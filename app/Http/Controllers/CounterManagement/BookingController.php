...
class BookingController extends Controller
{
    public function index(Request $request)
    {
        ...
        $companyName = CompanyModel::where('admin_id', Auth::guard('counter')->user()->admin_id)->first()->company_name;
        $companyDisplayName = CompanyModel::where('admin_id', Auth::guard('counter')->user()->admin_id)->first()->company_display_name;
        $companyAddress = CompanyModel::where('admin_id', Auth::guard('counter')->user()->admin_id)->first()->address1;
        return view('counter-management.booking', compact('screen', 'movie', 'date', 'time', 'seatData', 'schedule', 'seatCategories', 'temporaryReservedSeats', 'soldSeats', 'companyName', 'companyAddress', 'companyDisplayName', 'reservedSeats'));
    }
    ...
    public function checkBooking(Request $request)
    {
        ...
        if ($check->count() > 0) {
            ...
            if ($check->count() > 0) {
                ...
                $check = CounetrReservation::where('screen_id', $data['screen_id'])
                    ->where('movie_id', $data['movie_id'])
                    ->where('show_date', $data['show_date'])
                    ->where('show_time', $data['show_time'])
                    ->where('seat_name', $data['seat'])
                ...
                if($check->count() > 0)
                {
                    $seats = CounetrReservation::where('screen_id', $data['screen_id'])
                        ->where('movie_id', $data['movie_id'])
                        ->where('show_date', $data['show_date'])
                        ->where('show_time', $data['show_time'])
                        ->pluck('seat_name')->toArray();
...
//            data for tickets
            ...
            for ($i = 0; $i < $data['noOfSeatsChoosed']; $i++) {
                $dataToReservationDb['screen_id'] = $data['screen_id'];
                $dataToReservationDb['movie_id'] = $data['movie_id'];
                $dataToReservationDb['schedule_id'] = $data['schedule_id'];
                $dataToReservationDb['counter_id'] = Auth::guard('counter')->user()->id;
                $dataToReservationDb['show_date'] = $data['show_date'];
                $dataToReservationDb['show_time'] = $data['show_time'];
                $dataToReservationDb['show_day'] = ScheduledMovie::find($data['schedule_id'])->show_day;
                $dataToReservationDb['seat_name'] = $data['seatChoosedName'][$i];
                $dataToReservationDb['seat_category'] = $data['seatChoosedCategory'][$i];
                $dataToReservationDb['seat_price'] = $data['seatChoosedPrice'][$i];
                $dataToReservationDb['customer_name'] = $data['customer_name'];
                $dataToReservationDb['customer_pan_number'] = $data['customer_pan_num'];
                CounetrReservation::create($dataToReservationDb);
            }
